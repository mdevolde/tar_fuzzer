...
void test_tar_archive_longfile() {
    ...
    snprintf(header.size, 12, "%011o", 600);  // Size of the file in octal (12 bytes)
    ...
    FILE *file = fopen("test_archive_long.tar", "wb");
    if (file) {
        ...
        fclose(file);
        printf("The archive has been written to test_archive_long.tar\n");
    } else {
